...
import com.gsh.service.BalSerDao;
import com.gsh.service.ConSerDao;
import com.gsh.service.GuestSerDao;
import com.gsh.service.HotelSerDao;
import com.gsh.service.RTSerDao;
import com.gsh.service.RgoodSerDao;
...
public class RoomControl {
	...
	@Resource
	private GuestSerDao guestSerDao;
	@Resource
	private RoomCSerDao roomCSerDao;
	@Resource
	private ConSerDao conSerDao;
	@Resource
	private RTSerDao rtSerDao;
	@Resource
	private RgoodSerDao rgoodSerDao;
	@Resource
	private BalSerDao balSerDao;
	@Resource
	private HotelSerDao hotelSerDao;
	/**
	 * 
	...
	public Map<String, Object> getlist(Room r,HttpSession session,PageInfo p) throws Exception{
		...
		if(r.getRoomType()==null){
			...
			rt.setHid(Integer.parseInt(session.getAttribute("hid").toString()));
			r.setRoomType(rt);
			}
		//设默认值
		...
		map.put("total", roomSerDao.getTotal(r));
		map.put("rows", roomSerDao.findcri(r, p.getPage(), p.getRows()));
		return map;
	}
	...
	m.put("hid", Integer.parseInt(session.getAttribute("hid").toString()));
	map.put("rows",roomSerDao.searchbymap(m,p));
	return map;
	*/
	/**
	 * 
	...
	*@return “”
	...
	public String update_roomstate(int roomid,int state,String remark,HttpSession session,PageInfo p) throws Exception{
		Map<String, Object> map=new HashMap<String, Object>();
		Room r=roomSerDao.get(Room.class, roomid);
		if(remark!=null)
			r.setRemark(remark);
	...
	*@return “”
	...
	public String checkin(int roomid,Guest g,int paymethod,Double deposit,double roomprice,Consumption con,int hourday,HttpSession session){
		...
		con.setHid(Integer.parseInt(session.getAttribute("hid").toString()));
		conSerDao.save(con);
		Consumption con1=conSerDao.find(con).get(0);
		//房态修改
		r.setRoomstate(5);
		//是否为钟点房*******重点 ！后面计算房价需要！
	...
	public String checkout(int id,Model m){
		Map<String, Object> map=new HashMap<String, Object>();
		Room r=roomSerDao.get(Room.class, id);
		//房间号
		...
		c1.setChecked(0);
		Consumption con=conSerDao.find(c1).get(0);
		map.put("con",con);
	...
	*@return
	...
	public Map checkoutend(Balance bal,int conid,String roomnum,HttpSession session){
		Map<String, Object> map=new HashMap<String, Object>();
		Room r=roomSerDao.find("from Room as r where r.roomnum ="+"'"+roomnum+"'").get(0);
		r.setRoomstate(1);
		roomSerDao.save(r);
		Consumption con=conSerDao.get(Consumption.class, conid);
		con.setChecked(1);
		...
		bal.setHid(Integer.parseInt(session.getAttribute("hid").toString()));
		conSerDao.save(con);
		balSerDao.save(bal);
		return map;
	}
	...
	*@return
	...
	public String checkroomprice(int roomid){
		...
		c1.setRoomnum(roomnum);
		c1.setChecked(0);
		Consumption c=conSerDao.find(c1).get(0);
	...
	*@return
	...
	if(typeid==null){
		if(hid==null)
			hid=Integer.parseInt(session.getAttribute("hid").toString());
		...
		if("".equals(act)||null==act){
			...
			try {
				...
				rtlist=rtSerDao.find(rtype,p.getPage(),p.getRows());
			} catch (Exception e) {
				e.printStackTrace();
			}
			map.put("total", rtSerDao.find(rtype).size());
			map.put("rows", rtlist);
		}
		else {
			System.out.println("getrtype.do   hid="+hid);
			rtlist=rtSerDao.find("from RoomType as rt where rt.hid="+hid+"and rt.state = 1 ");
			map.put("rtlist", rtlist);
		}
	}
	//否则求此房型下所有可以预定的房间号
	else{
		RoomType rt=rtSerDao.get(RoomType.class, Integer.parseInt(typeid));
	...
	*@throws Exception
	...
	public Map<String, Object>  addgoon(Rgood rg,PageInfo p) throws Exception{
		Map<String, Object> map=new HashMap<String, Object>();
		List<Rgood> rglist=rgoodSerDao.find(rg, p.getPage(), p.getRows());
	...
	*@return
	...
	public String sureaddgood(int conid,int goodid,HttpServletRequest request){
		Rgood rg=rgoodSerDao.get(Rgood.class, goodid);
	...
	public Map<String, Object> searchinfo(Guest g) throws Exception{
		...
		List<Guest> glist=guestSerDao.findcri(g, 1, 10);
		if(glist.size()!=0){
			Guest sg=glist.get(0);
			Room room=roomSerDao.get(Room.class, sg.getRoomid());
	...
	public  Map<String, String> update(Room r,int roomtype,HttpSession session){
		 Map<String, String> map= new HashMap<String, String>();
		 Room room=roomSerDao.get(Room.class, r.getId());
		 room.setFloor(r.getFloor());
		 room.setWindow(r.getWindow());
		 room.setRelprice(r.getRelprice());
		 room.setHid(Integer.parseInt(session.getAttribute("hid").toString()));
	...
	public Map<String, Object> gethotel(){
		Map<String, Object> map=new HashMap<String, Object>();
		map.put("hlist", hotelSerDao.find("from Hotel"));
		return map;
	}	
