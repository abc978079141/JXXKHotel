			...
			<script type="text/javascript">  
					function rowformater(value, row, index) {  
					 ...
					 function getinfo(index) {
						  var row = $('#rolelist').datagrid('getData').rows[index];
						  var act="";
						  var img=$("#nodata").hide();
						  	roleid=row.roleid;
						  if(roleid==undefined){
...
var editIndex = undefined;
		function endEditing(){
			...
			if ($('#rolelist').datagrid('validateRow', editIndex)){
				...
				var roleinfo=$('#rolelist').datagrid('getRows')[editIndex];
		...
		function onDbClickRow(index){
			if (editIndex != index){
				...
				if (endEditing()){
					...
					/*S  前台和注册用户不可编辑*/
					var rolecode=$('#rolelist').datagrid('getRows')[index]['rolecode'];
				 	roleid=$('#rolelist').datagrid('getRows')[index].roleid;
		...
		function append(){
			if (endEditing()){
				...
				editIndex = $('#rolelist').datagrid('getRows').length-1;
				$('#rolelist').datagrid('selectRow', editIndex)
